...
public class CatGamePanel extends JPanel {
    ...
    private void handleSubmit() {
        ...
        if (model.getPlayerArrangement().isEmpty()) {
            showFeedback("Pick some letters first!", UITheme.ACCENT_YELLOW); return;
        }
        if (model.getPlayerArrangement().size() < model.getCurrentWord().length()) {
            showFeedback("Place all letters!", UITheme.ACCENT_YELLOW); return;
        }
    ...
    private void refreshGame() {
        ...
        if (d != CatGameModel.Difficulty.EASY) startWindEffect();
        else stopWindEffect();
        repaint();
    }
        ...
        java.awt.event.MouseAdapter ma = new java.awt.event.MouseAdapter() {
            ...
            public void mouseReleased(java.awt.event.MouseEvent e) {
                ...
                if (currentScreen == Screen.INTRO) {
                    ...
                    if (r != null && r.contains(mx, my) && !isDragging) startGame();
                    isDragging = false;
                    return;
                }
                if (currentScreen != Screen.GAME || waitingForNext) { isDragging = false; return; }
