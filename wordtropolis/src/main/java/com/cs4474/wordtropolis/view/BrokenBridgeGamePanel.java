 ...
 *
 * Mirrors CatGamePanel behaviour exactly:
 *  • Intro card matches cat-game style (title + 2 lines + divider + button)
 *  • Hero: correct avatar, centered under bridge, sprite-sheet animated
 *  • Hero is clipped to below the bridge so no artifact shows through the gap
 *  • No NPCs
 *  • Timer timeout → "Time's up! Try again." + clear + restart (no game-over)
 *  • Check before all words placed → "Pick some words first!" feedback
 *  • Enter key triggers CHECK (same as CatGamePanel Enter → submit)
 *  • Left panel: vertical green progress bar (same as CatGamePanel.paintLeftPanel)
 *  • After all rounds: score dialog → SCREEN_MAP  (same as CatGamePanel.showResultDialog)
 *  • Hero re-loaded in refresh() so the chosen avatar always appears
 *  • Word count increases per round via model.getWordsThisRound()
 *  • Sound: BGM on start, box on tile place, error on wrong, cha-ching on correct
 */
public class BrokenBridgeGamePanel extends JPanel implements Refreshable {
    ...
    // ── Screen state ──────────────────────────────────────────────────────────
    private enum Screen { INTRO, PLAYING, CORRECT_ANIM, ERROR_ANIM }
    ...
    public BrokenBridgeGamePanel() {
        ...
        addMouseListener(new MouseAdapter() {
            @Override public void mouseReleased(MouseEvent e) { handleClick(e.getX(), e.getY()); }
        });
    ...
    public void refresh() {
        ...
        if (heroTimer != null && !heroTimer.isRunning()) heroTimer.start();
    ...
    private BufferedImage loadHeroImage() {
        ...
        if ("Mia".equalsIgnoreCase(avatar))  return img("/images/general/hero1_standing.png");
        if ("Zyx".equalsIgnoreCase(avatar))  return img("/images/general/alien_standing.png");
    ...
    private void showFeedback(String msg, Color c) {
        ...
        feedbackTimer = new Timer(3000, e -> {
            feedbackMsg = ""; repaint(); ((Timer) e.getSource()).stop();
        });
        feedbackTimer.start();
        repaint();
    }
    ...
    private void shakeEffect() {
        ...
        shakeTimer = new Timer(40, e -> {
            if (++shakeTick > 8) { shakeTimer.stop(); repaint(); } else repaint();
        });
        shakeTimer.start();
    }
    ...
    private void handleClick(int mx, int my) {
        if (screen == Screen.INTRO) {
            ...
            if (b != null && b.contains(mx, my)) {
                stopTimers(); Wordtropolis.showScreen(Wordtropolis.SCREEN_MAP);
            } else if (s != null && s.contains(mx, my)) {
                startGame();
            }
            return;
        }
        if (screen == Screen.CORRECT_ANIM || screen == Screen.ERROR_ANIM) return;

        Rectangle back  = clickZones.get("back_btn");
        Rectangle check = clickZones.get("check_btn");
        Rectangle clear = clickZones.get("clear_btn");

        if (back  != null && back.contains(mx, my))  { stopTimers(); Wordtropolis.showScreen(Wordtropolis.SCREEN_MAP); return; }
        if (clear != null && clear.contains(mx, my)) { model.clearSelection(); repaint(); return; }
        if (check != null && check.contains(mx, my)) { handleCheck(); return; }
        ...
        for (int i = 0; i < sel.size(); i++) {
            ...
            if (mx >= sx && mx <= sx + TILE_W && my >= slotY && my <= slotY + TILE_H) {
                model.deselectWord(sel.get(i)); repaint(); return;
            }
        }
    ...
    private void handleCheck() {
        ...
        if (model.isAnswerCorrect()) {
            ...
            if (model.isComplete()) {
                if (gameTimer != null) gameTimer.stop();
                new Timer(900, e -> { showResultDialog(); ((Timer) e.getSource()).stop(); }).start();
            } else {
            ...
            model.clearSelection();
            screen = Screen.ERROR_ANIM; errorFrame = 0;
        }
    }
    ...
    private void triggerSparkle() {
        ...
        sparkleFrame = 0; showSparkle = true;
        if (sparkleTimer != null) sparkleTimer.stop();
    ...
    private void paintBridge(Graphics2D g2, int w, int h) {
        ...
        if (gapLeft < cliffSt) {
            g2.setColor(new Color(0, 0, 0, 88)); g2.fillRect(gapLeft, bridgeY, cliffSt - gapLeft, bridgeH);
            g2.setColor(new Color(0, 0, 0, 50)); g2.fillRect(gapLeft, bridgeY + bridgeH, cliffSt - gapLeft, 8);
        }
    ...
    private void drawDeck(Graphics2D g2, int x, int y, int width, int height, boolean wood) {
        ...
        if (wood) {
            for (int px = x; px < x + width; px += 13) {
                ...
                g2.setColor(new Color(160, 108, 48)); g2.fillRect(px, y, pw, height);
                g2.setColor(new Color(130, 85,  35)); g2.fillRect(px, y,  2, height);
                g2.setColor(new Color(195, 145, 75)); g2.fillRect(px + 3, y + 2, Math.max(0, pw - 5), 4);
            }
        } else {
            g2.setColor(new Color(105, 90, 75)); g2.fillRect(x, y, width, height);
        ...
        g2.setColor(new Color(205, 185, 158)); g2.fillRect(x, y,              width, 3);
        g2.setColor(new Color(55,  42,  30));  g2.fillRect(x, y + height - 3, width, 3);
    }

    private void drawTower(Graphics2D g2, int x, int y, int w, int h) {
        g2.setColor(new Color(85,  70, 58));   g2.fillRect(x, y, w, h);
        g2.setColor(new Color(125, 110, 95));  g2.fillRect(x, y, 3, h);
        g2.setColor(new Color(55,  44,  34));  g2.fillRect(x + w - 3, y, 3, h);
        g2.setColor(new Color(70,  58,  46));  g2.fillRect(x - 5, y, w + 10, 9);
        g2.setColor(new Color(145, 128, 108)); g2.fillRect(x - 5, y, w + 10, 3);
    }
    ...
    private void drawQuadRope(Graphics2D g2, int x1, int y1, int cx, int cy, int x2, int y2) {
        ...
        for (int i = 1; i <= 70; i++) {
            ...
            g2.drawLine(px, py, nx, ny); px = nx; py = ny;
        }
    }
    ...
    private void paintTimerBar(Graphics2D g2, int W) {
        ...
        if (imgProgressBar != null) {
            ...
            g2.setColor(c); g2.fillRoundRect(tbX, tbY, fillW, tbH, 8, 8);
        }
    ...
    private void paintWordTiles(Graphics2D g2, int w, int h) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (!placed) tileRects.put(word, new Rectangle(tx, tileY, TILE_W, TILE_H));
    ...
    private void paintBottomUI(Graphics2D g2, int w, int h) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (imgIndividualBox != null)
                ...
                g2.setColor(new Color(255, 248, 222)); g2.fillRoundRect(sx, slotY, TILE_W, TILE_H, 10, 10);
                g2.setColor(new Color(200, 172, 102)); g2.drawRoundRect(sx, slotY, TILE_W, TILE_H, 10, 10);
            }

            if (i < selected.size()) {
                Font f = gameFont(14f); FontMetrics fm = g2.getFontMetrics(f);
                g2.setFont(f); g2.setColor(new Color(48, 32, 10));
    ...
    private void paintOuterBox(Graphics2D g2, int x, int y, int w, int h) {
        g2.setColor(new Color(229, 226, 207)); g2.fillRect(x, y, w, h);
        if (imgOuterBox != null) {
            ...
            g2.setColor(new Color(0xC8A97A)); g2.setStroke(new BasicStroke(2));
            g2.drawRoundRect(x + 1, y + 1, w - 2, h - 2, 6, 6); g2.setStroke(new BasicStroke(1));
        }
    }
    ...
    private void paintOuterBoxButton(Graphics2D g2, int x, int y, int w, int h, String label, String zoneId) {
        ...
        if (!zoneId.isEmpty()) clickZones.put(zoneId, new Rectangle(x, y, w, h));
    }
    ...
    private void paintClickBox(Graphics2D g2, int x, int y, int w, int h,
        ...
        if (imgOuterBox != null) { g2.setColor(new Color(1f, 1f, 1f, 0.15f)); g2.fillRoundRect(x, y, w, h, 8, 8); }
        g2.setColor(new Color(0, 0, 0, 100)); g2.setStroke(new BasicStroke(2));
        g2.drawRoundRect(x + 1, y + 1, w - 2, h - 2, 8, 8); g2.setStroke(new BasicStroke(1));
        g2.setFont(font); g2.setColor(textColor);
    ...
    private Font gameFont(float size) {
        if (size >= 17) return UITheme.FONT_HEADING.deriveFont(size);
        if (size >= 14) return UITheme.FONT_LETTER.deriveFont(size);
