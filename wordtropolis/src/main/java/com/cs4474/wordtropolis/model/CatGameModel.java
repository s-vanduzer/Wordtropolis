...
public class CatGameModel {
    ...
    /**
     * Result codes returned by typeKey().
     * CatGamePanel uses these to decide what feedback to show.
     */
    ...
    /**
     * Create the model using the word list provided by the teacher
     * via Game.getInstance().getWordList().
     *
     * We do NOT inject extra default words on top — we use exactly
     * what is provided. The only exception is if the list arrives
     * completely empty, in which case an emergency set is used so
     ...
     *
     * Rules (in order):
     *  1. Uppercase + trim every entry, skip anything non-alphabetic
     *  2. Remove duplicates (keep first occurrence)
     *  3. Sort shortest → longest (easier words first)
     *  4. Cap at TOTAL_WORDS entries
     *  5. If fewer than TOTAL_WORDS, cycle the list to fill the gap
     *  6. Emergency fallback only if list was completely empty
     */
    ...
    private void updateDifficulty() {
        if      (wordsCompleted < 2) currentDifficulty = Difficulty.EASY;
        else if (wordsCompleted < 4) currentDifficulty = Difficulty.MEDIUM;
    ...
    /**
     * Move a tile from the available pool into the arrangement.
     * Called when the player clicks an available letter tile.
     *
    ...
    /**
     * Move a tile from the arrangement back to the pool.
     * Called when the player clicks an already-placed tile.
     *
    ...
    /**
     * Swap two tiles within the arrangement.
     * Called when the player drags one placed tile onto another slot.
     *
     ...
     *
     * Logic:
     *  1. Ignore anything that is not a letter (digits, punctuation, etc.)
     *  2. If the arrangement is already full → TYPED_FULL
     *  3. Search availableLetters for the FIRST tile matching this letter
     *       found  → move it into arrangement → TYPED_OK
     *              (CatGamePanel plays tile-click sound + same tile effect)
     *       not found → TYPED_NOT_AVAILABLE
     *              (CatGamePanel shakes input area + shows "not an option")
     *
     ...
     * Removes the LAST placed tile and returns it to the available pool.
     * Identical in effect to clicking the last tile in the arrangement row.
     *
     ...
     * Incorrect → increments incorrectAttempts, deducts penalty,
     *             records misspelling, returns all tiles to pool
     *
    ...
    public int             getLadderRungs()       { return TOTAL_WORDS; }
    ...
    public double          getProgress()          { return (double) wordsCompleted / TOTAL_WORDS; }
    public boolean         isComplete()           { return wordsCompleted >= TOTAL_WORDS; }
}