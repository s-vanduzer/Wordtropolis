 ...
 *
 * Timer behaviour: when it hits zero the panel clears the selection and resets
 * the timer (same as CatGamePanel "Time's up – try again"). Game ends only when
 * bridgeProgress reaches TOTAL_ROUNDS.
 *
 * Word count increases as the bridge is repaired: Rounds 1–2 → 3 words Rounds
 * 3–4 → 4 words Rounds 5–6 → 5 words
 */
public class BrokenBridgeGameModel {
    ...
    /**
     * Maximum words ever shown — used only to guarantee wordPool is large
     * enough.
     */
    ...
    // ── Difficulty ramp ───────────────────────────────────────────────────────
    /**
     * How many words to order this round — increases as the bridge gets
     * repaired. bridgeProgress 0–1 → 3 words (rounds 1–2) bridgeProgress 2–3 →
     * 4 words (rounds 3–4) bridgeProgress 4–5 → 5 words (rounds 5–6)
     */
    ...
    public final void restartGame() {
        ...
        // 2. This will now pull a fresh set of words from the shuffled pool
        newRound();
    }

    /**
     * Reset the countdown to a full round. Called on timeout or after a correct
     * round.
     */
    ...
    public boolean deselectWord(String word) {
        return selected.remove(word);
    }
    ...
    public boolean isAnswerCorrect() {
        ...
        for (int i = 0; i < selected.size(); i++) {
            if (!selected.get(i).equals(correctOrder.get(i))) {
                return false;
            }
        }
        correctAttempts++;
    ...
    public List<String> getDisplayWords() {
        return Collections.unmodifiableList(displayWords);
    }

    public List<String> getCorrectOrder() {
        return Collections.unmodifiableList(correctOrder);
    }
    ...
    public boolean isComplete() {
        return bridgeProgress >= TOTAL_ROUNDS;
    }
